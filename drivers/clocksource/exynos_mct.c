...
static void exynos4_mct_frc_start(void)
{
	...
	reg = readl_relaxed(EXYNOS4_MCT_G_TCON);
...
{
	return readl_relaxed(EXYNOS4_MCT_G_CNT_L);
}

static cycle_t exynos4_frc_read(struct clocksource *cs)
{
	unsigned int lo, hi;
	u32 hi2 = readl_relaxed(EXYNOS4_MCT_G_CNT_U);

	do {
		hi = hi2;
		lo = readl_relaxed(EXYNOS4_MCT_G_CNT_L);
		hi2 = readl_relaxed(EXYNOS4_MCT_G_CNT_U);
	} while (hi != hi2);
...
static void exynos4_mct_comp0_stop(void)
{
	...
	tcon = readl_relaxed(EXYNOS4_MCT_G_TCON);
				   ...
				   struct mct_clock_event_device *mevt)
{
	...
	tmp = readl_relaxed(mevt->base + MCT_L_TCON_OFFSET);
...
static void exynos4_mct_tick_clear(struct mct_clock_event_device *mevt)
{
	...
	if (readl_relaxed(mevt->base + MCT_L_INT_CSTAT_OFFSET) & 1)
		exynos4_mct_write(0x1, mevt->base + MCT_L_INT_CSTAT_OFFSET);
}
